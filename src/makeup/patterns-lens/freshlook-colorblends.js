// src/eyewear/patterns-lens/freshlook-colorblends.js

import { lerp, shadeColor, PERLIN_PERMUTATION } from "./utils";

function createFreshlookNoise2D() {
...
export function drawFreshlookColorBlendsLens(
  canvasCtx,
  centerX,
  centerY,
  radius,
  ...
  const noise2D = createFreshlookNoise2D();
  const scale = 0.04;
  ...
  for (let y = centerY - radius; y < centerY + radius; y++) {
    for (let x = centerX - radius; x < centerX + radius; x++) {
      ...
      if (dx * dx + dy * dy <= radius * radius) {
        ...
        noiseValue +=
          ((noise2D(x * detailScale, y * detailScale) + 1) / 2) * 0.4;

        // الگوی تازه‌ برای Freshlook با جزئیات مختلف
        noiseValue += Math.sin(angle * 12) * 0.2;
        noiseValue += Math.cos(dist * Math.PI * 4) * 0.1;

        const colorValue = lerp(noiseValue, 0.25, 0.75);
  ...
  addFreshlookDetails(canvasCtx, centerX, centerY, radius);
}

function addFreshlookDetails(canvasCtx, centerX, centerY, radius) {
  ...
  for (let i = 0; i < detailCount; i++) {
    ...
    const subDetailCount = 5;
    for (let j = 0; j < subDetailCount; j++) {
      const subAngle = angle + (Math.random() - 0.5) * angleStep * 0.6;
